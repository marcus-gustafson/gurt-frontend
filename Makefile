...
unit:
>pipenv run pytest apps/api/tests -q
lint:
>pipenv run ruff check .
>npm --prefix apps/web run lint
typecheck:
>pipenv run mypy apps/api || true
...
e2e-real:
>OPENAI_BASE_URL=$$OPENAI_BASE_URL OPENAI_API_KEY=$$OPENAI_API_KEY npm --prefix apps/web run build
>OPENAI_BASE_URL=$$OPENAI_BASE_URL OPENAI_API_KEY=$$OPENAI_API_KEY npm --prefix apps/web run start -- -p 3000 & echo $$! > .web.pid; sleep 3
